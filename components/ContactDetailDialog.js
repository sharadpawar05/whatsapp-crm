...
// Import the new Reminder Dialog
import SetReminderDialog from './SetReminderDialog'
import { Phone, MessageCircle, Calendar, Edit2, Check, X, Plus, Bell } from 'lucide-react'
...
export default function ContactDetailDialog({ contact, open, onClose, onUpdate }) {
  ...
  // ... (handleUpdateName and handleAddTag functions remain the same)
        ...
        <DialogContent className="max-w-lg">
            ...
            <div className="flex items-center gap-2">
              {isEditingName ? (
                ...
                <DialogTitle className="flex items-center gap-2 group">
                  {contact.name}
                  <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100" onClick={() => setIsEditingName(true)}><Edit2 className="w-3 h-3" /></Button>
                </DialogTitle>
              )}
            </div>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap gap-2">
                {contact.tags?.map((tag) => (
                  <span key={tag} className={`px-2 py-1 text-[10px] font-bold uppercase rounded-md ${TAG_COLORS[tag] || 'bg-purple-100 text-purple-700'}`}>
                    {TAG_LABELS[tag] || `✨ ${tag}`}
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <Input placeholder="Add custom tag (e.g. VIP)" value={newTag} onChange={(e) => setNewTag(e.target.value)} className="h-8 text-xs" onKeyDown={(e) => e.key === 'Enter' && handleAddTag()} />
                <Button size="sm" variant="secondary" onClick={handleAddTag} className="h-8"><Plus className="w-3 h-3 mr-1" /> Add</Button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold flex items-center gap-2"><Calendar className="w-4 h-4" /> Notes History</h3>
              <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
                {notesList.length === 0 ? <p className="text-sm text-muted-foreground italic">No notes yet.</p> : notesList.map((noteText, index) => (
            ...
            <div className="pt-2 border-t space-y-3">
              <Textarea placeholder="Add a new update..." value={newNote} onChange={(e) => setNewNote(e.target.value)} className="min-h-[80px]" />
